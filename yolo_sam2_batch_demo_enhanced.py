...
OUTPUT_DIR = Path("outputs/yolo_sam2_batch_demo_enhanced")
...
for img_path in tqdm(image_paths, desc="Processing images"):
    ...
    if len(input_boxes) > 0:
        ...
        cv2.imwrite(os.path.join(img_output_dir, f"{img_name}_enhanced.jpg"), enhanced_image)
        
        # Save original image for reference
        original_image = cv2.imread(img_path)
        cv2.imwrite(os.path.join(img_output_dir, f"{img_name}_original.jpg"), original_image)
        
        # First, get SAM2 results on original image for comparison
        print("Getting SAM2 results on original image...")
        sam2_predictor.set_image(image_source)  # Use original image
        ...
        original_masks, original_scores, original_logits = sam2_predictor.predict(
            point_coords=None,
            point_labels=None,
            box=input_boxes,
        ...
        # Then, get SAM2 results on enhanced image
        ...
        """
        # convert the shape to (n, H, W) for both original and enhanced masks
        if original_masks.ndim == 4:
            original_masks = original_masks.squeeze(1)
        ...
        """
        # Save visualization on original image with original SAM2 results
        original_img = cv2.imread(img_path)
        original_detections = sv.Detections(
            xyxy=input_boxes,
            mask=original_masks.astype(bool),  # Using original masks from original image
            class_id=class_ids
        )
        
        box_annotator = sv.BoxAnnotator()
        original_annotated = box_annotator.annotate(scene=original_img.copy(), detections=original_detections)

        label_annotator = sv.LabelAnnotator()
        original_annotated = label_annotator.annotate(scene=original_annotated, detections=original_detections, labels=labels)
        ...
        if DUMP_JSON_RESULTS:
            # convert mask into rle format - using enhanced masks as main results
            enhanced_mask_rles = [single_mask_to_rle(mask) for mask in enhanced_masks]
            original_mask_rles = [single_mask_to_rle(mask) for mask in original_masks]
            ...
            results = {
                ...
                "original_annotations" : [
                    ...
                    for class_name, box, mask_rle, score in zip(class_names, input_boxes, original_mask_rles, original_scores)
                ],
                "box_format": "xyxy",
                "img_width": w,
                "img_height": h,
