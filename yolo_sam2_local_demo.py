...
Hyper parameters
"""
IMG_PATH = "customdata_830/images/vaild/00031.jpg"  # 使用 predict_shrimp.py 中的同一張圖片
...
OUTPUT_DIR = Path("outputs/yolo_sam2_local_demo")
...
# setup the input image for SAM 2 and YOLOv7
img_path = IMG_PATH
...
if len(input_boxes) > 0:
    ...
    cv2.imwrite(os.path.join(OUTPUT_DIR, "yolov7_annotated_image.jpg"), annotated_frame)

    mask_annotator = sv.MaskAnnotator()
    annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)
    cv2.imwrite(os.path.join(OUTPUT_DIR, "yolo_sam2_annotated_image_with_mask.jpg"), annotated_frame)
    ...
    if DUMP_JSON_RESULTS:
        ...
        with open(os.path.join(OUTPUT_DIR, "yolo_sam2_local_image_demo_results.json"), "w") as f:
            json.dump(results, f, indent=4)

else:
    print("No objects detected, skipping SAM2 segmentation")
